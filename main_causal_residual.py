...
AP_COORDS_MAP = {
    ...
    "corridor":        {2:(17.0,6.8),3:(38.0,9.0),4:(48.0,5.0),5:(2.0,7.5)}, # AP1 dropped
    "building": {
...
# ---------- Only RF-RMSE (per-axis mean MSE, then sqrt) ----------
...
class CausalTemporalBlock(nn.Module):
    """
    CAUSAL Temporal Block with WeightNorm (Bai et al., 2018 style)
    ...
    def forward(self, x):
        ...
        # crop to input length (strict causality, no right-side extension)
...
class CausalGCN_TCN_PerScan(nn.Module):
    ...
    def __init__(self, N, in_feats, hidden=128, gcn_layers=2, tcn_blocks=4,
        ...
        self.legacy_both = (use_gcn and use_tcn)  # mimic original "both"
        ...
        # Input path: legacy_both uses NO pre-proj 
        if self.legacy_both:
        ...
        # CAUSAL TCN stack (WeightNorm enabled inside each block)
    ...
    def forward(self, x, return_weights=False):
        ...
        if self.legacy_both:
            # ORIGINAL behavior: gate BEFORE GCN
            h = x * w.unsqueeze(-1)
        else:
            # GCN-only and TCN-only get an in-projection first
            h = self.in_proj(x)
...
# ---- ptflops helper (module scope) ----
def print_complexity_with_ptflops(model, t_len, n_ap, feat_dim, device, out_dir, print_per_layer=False):
    """
    Profiles MACs/params with ptflops using a wrapper that forwards model(x).
    ptflops returns MACs; FLOPs ≈ 2 * MACs is a common convention.
    """
    try:
    ...
    class _Wrap(nn.Module):
        ...
        def forward(self, x):
            return self.m(x)  # your model already supports model(x)

    wrap = _Wrap(model).to(device)
    input_res = (t_len, n_ap, feat_dim)  # (T, N, F) — ptflops adds batch=1

    # Put model in eval() to avoid dropout randomness
...
# ============================================================
# Train/Eval per env - CAUSAL VERSION
# ============================================================
def run_env(env_name: str, cfg_base: dict, feature_mode: str, model_type: str,
    ...
    """
    model_type ∈ {'gcn','tcn','both'}
    CAUSAL VERSION: Uses CausalGCN_TCN_PerScan
    """
    ...
    # Rolling stats per requested windows
    vars_to_process = []
    if feature_mode == "rss":  vars_to_process = [(2, 1)]
    elif feature_mode == "rtt": vars_to_process = [(2, 1)]
    elif feature_mode == "both": vars_to_process = [(2, 1), (5, 4)]
    ...
    cfg = dict(cfg_base)
    # CAUSAL MODEL: Use CausalGCN_TCN_PerScan instead of GCN_TCN_PerScan
    ...
    # --- ptflops profile for this (env, feature_mode, model_type) ---
    T_len   = int(Xtr.shape[1])
    N_ap    = int(Xtr.shape[2])
    featdim = int(Xtr.shape[3])
    print_complexity_with_ptflops(
        model,
        t_len=T_len,
        n_ap=N_ap,
        feat_dim=featdim,
        device=device,
        out_dir=out_dir,          # profile gets written to artifacts_causal_* folder
        print_per_layer=False     # set True if you want per-layer table
    )
    ...
    for epoch in range(1, epochs+1):
        ...
        for xb, yb in train_loader:
            ...
            with autocast(enabled=USE_AMP):
                ...
                if feature_mode in ["rss", "both"]: rss_mask_idx = 1
                if feature_mode == "rtt": rtt_mask_idx = 1
                elif feature_mode == "both": rtt_mask_idx = 4
                valid_sum = torch.zeros_like(xb[...,0])
                if rss_mask_idx != -1: valid_sum += xb[..., rss_mask_idx]
                if rtt_mask_idx != -1: valid_sum += xb[..., rtt_mask_idx]
    ...
    # Save per-env summary CSV (rf_rmse only)
    row = {
        "env": env_name,
        "feature_mode": feature_mode,
        "model_type": model_type,
        "rf_rmse": float(final_rf_rmse),
        "N_AP": N, "grid_m": grid_m, "T_used": int(Xtr.shape[1]),
        "feat_dim": int(Xtr.shape[-1]),
        "causal": True,  # Mark as causal version
    }
    ...
    # Optional plot of RF-RMSE vs epoch for this run
    try:
...
# ============================================================
# Main: CAUSAL VERSION sweep
# ============================================================
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    from datetime import datetime

    ENVS       = ["corridor", "office", "lecture_theatre", "building"]
    # ENVS       = ["building"]
    MODES      = ["rss", "rtt", "both"]
    MODELTYPES = ["gcn", "tcn", "both"]

    # CAUSAL VERSION sweep: 1500 epochs, rolls [5,10,20]
    SWEEPS = [(500, [5, 10, 20])]
    EVAL_EVERY = 200

    for total_epochs, rolls in SWEEPS:
        stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        sweep_tag = f"causal_epochs{total_epochs}_rolls{'-'.join(map(str, rolls))}"
        print(f"\n========== CAUSAL SWEEP START: {sweep_tag} ==========\n")

        all_rows = []
        distances_for_plot = {(env, mt): {} for env in ENVS for mt in MODELTYPES}

        sweep_dir = RESULTS_ROOT / f"results_causal_sweep_{sweep_tag}_{stamp}"
        ...
        for env_name in ENVS:
            ...
            for model_type in MODELTYPES:
                for mode in MODES:
                    row, dists = run_env(
                        env_name, FIXED_CFG, feature_mode=mode, model_type=model_type,
                        epochs=total_epochs, eval_every=EVAL_EVERY,
                        roll_windows=rolls, out_suffix=sweep_tag
                    )
                    all_rows.append({**row, "epochs": total_epochs, "roll_windows": str(rolls)})
                    distances_for_plot[(env_name, model_type)][mode] = dists

                # Per-env CDF (fixed model_type), comparing feature modes
                try:
                    plt.figure(figsize=(7.5, 5.0))
                    for mode in MODES:
                        d = np.sort(distances_for_plot[(env_name, model_type)].get(mode, np.array([])))
                        if d.size == 0: continue
                        xs = d
                        ys = np.arange(1, d.size + 1) / d.size
                        plt.plot(xs, ys, label=mode.upper())
                    plt.xlabel("Error distance (m)"); plt.ylabel("CDF")
                    plt.title(f"CAUSAL CDF of Localization Error – {env_name.replace('_',' ').title()} | {model_type.upper()} ({sweep_tag})")
                    plt.grid(True, linestyle="--", alpha=0.5)
                    plt.legend(); plt.tight_layout()
                    plt.savefig(sweep_dir / f"cdf_causal_{env_name}_{model_type}_{sweep_tag}.png", dpi=150)
                    plt.close()
                except Exception:
                    pass
        ...
        out_csv = sweep_dir / f"artifacts_causal_all_envs_all_models_all_modes_{sweep_tag}.csv"
        df_all.to_csv(out_csv, index=False)
        print("\n=== CAUSAL SWEEP COMPLETE ===")
        print(df_all)
        print(f"\nCombined CSV saved to: {out_csv}")

        # Best (by RF-RMSE) per (environment, model)
        try:
            best = (
                df_all.sort_values("rf_rmse")
                      .groupby(["env","model_type"], as_index=False)
                      .first()[["env","model_type","feature_mode","rf_rmse","epochs","roll_windows"]]
            )
            print("\n=== Best feature mode per (environment, model) - CAUSAL ===")
            print(best.to_string(index=False))
            best.to_csv(sweep_dir / f"best_mode_per_env_model_causal_{sweep_tag}.csv", index=False)
        except Exception:
            pass

        # Aggregated CDF across all environments per model_type
        try:
            for model_type in MODELTYPES:
                plt.figure(figsize=(7.5, 5.0))
                for mode in MODES:
                    stacks = [distances_for_plot[(env, model_type)].get(mode, np.array([])) for env in ENVS]
                    agg = np.concatenate([s for s in stacks if s.size > 0], axis=0)
                    if agg.size == 0: continue
                    d = np.sort(agg)
                    xs = d
                    ys = np.arange(1, d.size + 1) / d.size
                    plt.plot(xs, ys, label=f"{mode.upper()} (N={d.size})")
                plt.xlabel("Error distance (m)"); plt.ylabel("CDF")
                plt.title(f"CAUSAL CDF – All Environments (Aggregated) | {model_type.upper()} [{sweep_tag}]")
                plt.grid(True, linestyle="--", alpha=0.5)
                plt.legend(); plt.tight_layout()
                plt.savefig(sweep_dir / f"cdf_causal_all_envs_{model_type}_{sweep_tag}.png", dpi=150)
                plt.close()
        except Exception:
            pass

        print(f"\n========== CAUSAL SWEEP END: {sweep_tag} ==========\n")
